...
updateShippingProgress(){const freeShippingThreshold=CONFIG.FREE_SHIPPING_THRESHOLD;const subtotal=this.app.managers.cart.calculateSubtotal();const remaining=Math.max(0,freeShippingThreshold-subtotal);const shippingRemaining=document.getElementById('shippingRemaining');const shippingProgressFill=document.getElementById('shippingProgressFill');if(!shippingRemaining||!shippingProgressFill)return;const today=new Date();const currentDay=today.getDay();const isFreeShippingDay=currentDay===4||currentDay===5;if(subtotal>=freeShippingThreshold&&isFreeShippingDay){shippingRemaining.textContent='¡Envío gratis desbloqueado! (Hoy es día de envío gratis)';shippingProgressFill.style.width='100%';shippingProgressFill.style.background='var(--success-green)'}else if(subtotal>=freeShippingThreshold){shippingRemaining.textContent=`Compra de $${subtotal.toLocaleString()} - Envío a consultar (no es jueves/viernes)`;shippingProgressFill.style.width='100%';shippingProgressFill.style.background='var(--warning-orange)'}else{if(isFreeShippingDay){shippingRemaining.textContent=`¡Faltan $${remaining.toLocaleString()} para envío gratis HOY!`}else{shippingRemaining.textContent=`¡Faltan $${remaining.toLocaleString()} para envío gratis (jueves/viernes)!`}
const progress=(subtotal/freeShippingThreshold)*100;shippingProgressFill.style.width=`${progress}%`;shippingProgressFill.style.background='var(--accent-red)'}
if(this.app.cart.length===0){shippingRemaining.textContent=`¡Faltan $${freeShippingThreshold.toLocaleString()} para envío gratis los jueves y viernes!`;shippingProgressFill.style.width='0%'}}
isFreeShippingDay(day){return day==='jueves'||day==='viernes'}
getCurrentDayName(){const days=['domingo','lunes','martes','miércoles','jueves','viernes','sábado'];const today=new Date();return days[today.getDay()]}
...
updateShippingProgressStep1(){const freeShippingThreshold=CONFIG.FREE_SHIPPING_THRESHOLD;const subtotal=this.app.managers.cart.calculateSubtotal();const remaining=Math.max(0,freeShippingThreshold-subtotal);const shippingRemaining=document.getElementById('shippingRemainingStep1');const shippingProgressFill=document.getElementById('shippingProgressFillStep1');if(!shippingRemaining||!shippingProgressFill)return;const today=new Date();const currentDay=today.getDay();const isFreeShippingDay=currentDay===4||currentDay===5;if(subtotal>=freeShippingThreshold&&isFreeShippingDay){shippingRemaining.innerHTML=`
                <div class="free-shipping-icon">
                    <i class="fas fa-check-circle"></i>
                    <span>¡Envío gratis desbloqueado! (Hoy es día de envío gratis)</span>
                </div>
            `;shippingProgressFill.style.width='100%'}else if(subtotal>=freeShippingThreshold){shippingRemaining.innerHTML=`
            ...
            `;shippingProgressFill.style.width='100%'}else{const progress=(subtotal/freeShippingThreshold)*100;if(isFreeShippingDay){shippingRemaining.textContent=`¡Faltan $${remaining.toLocaleString()} para envío gratis HOY!`}else{shippingRemaining.textContent=`¡Faltan $${remaining.toLocaleString()} para envío gratis (jueves/viernes)!`}
