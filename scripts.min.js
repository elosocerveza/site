...
const transformedData=beerArray.map(item=>{let subcategory=item.subcategory||'otros';subcategory=subcategory.toLowerCase();return{id:item.id||0,name:item.name||'',subname:item.subname||'',description:item.description||'',price:parseInt(item.price)||0,category:'beers',subcategory:subcategory||'',image:item.image||'',badge:item.badge||'',stock:parseInt(item.stock)||0,sold:parseInt(item.sold)||0,rating:parseFloat(item.rating)||0,active:item.active||!1,abv:parseFloat(item.abv)||0,ibu:parseInt(item.ibu)||0,ingredients:item.ingredients||'',size:item.size||''}});console.log('Cervezas transformadas:',transformedData.length);return transformedData}catch(error){console.error('Error obteniendo datos desde URL:',error);throw error}}
...
await refreshBeersData()}catch(error){console.error('Error cargando datos de cervezas:',error);if(isBeersPage){const container=document.getElementById('beers-container');if(container){container.innerHTML=`
                ...
                `}}}}
async function refreshBeersData(){try{console.log('Forzando actualización de datos...');const transformedData=await fetchBeersFromURL();beersData=transformedData;setCachedBeers(transformedData);if(beersData.length>0){generateProductSchema(beersData)}
...
function formatStock(stock){if(stock===0)return'Sin stock';if(stock<10)return `Últimas ${stock} botellas`;if(stock<50)return `${stock} disponibles`;return `${stock}+ en stock`}
...
function getProductBadge(beer){if(beer.stock===0){return'<div class="product-badge badge-sold-out">AGOTADA</div>'}else if(beer.stock<10){return'<div class="product-badge badge-low-stock">ÚLTIMAS BOTELLAS</div>'}else if(beer.sold>200){return'<div class="product-badge badge-popular">POPULAR</div>'}else if(beer.sold==0){return'<div class="product-badge badge-new">NUEVO</div>'}
...
cartEmpty.style.display='none';const total=cart.reduce((sum,item)=>sum+(item.price*item.quantity),0);cartTotal.textContent=`$${total.toLocaleString('es-AR')}`;cartItems.innerHTML=cart.map((item,index)=>{const beerData=beersData.find(beer=>beer.id==item.id);const currentStock=beerData?beerData.stock:0;const maxQuantity=currentStock;return `
        <div class="cart-item">
...
function addToCart(id,name,price,image,subname='',size=''){const beerData=beersData.find(beer=>beer.id==id);if(!beerData){showNotification(`Error: Producto no encontrado`);return}
const currentStock=beerData.stock;if(currentStock<=0){showNotification(`Lo sentimos, ${name} ${subname} está agotado`);return}
...
cart.push({id:id,name:name,subname:subname,size:size,price:price,image:image,quantity:1})}
updateCartCount();showNotification(`${name} ${subname} añadido al carrito`);if(document.getElementById('cart-sidebar').classList.contains('active')){renderCart()}}
...
if(e.target.closest('.increase')){const button=e.target.closest('.increase');if(button.disabled)return;const index=parseInt(button.getAttribute('data-index'));const item=cart[index];const beerData=beersData.find(beer=>beer.id==item.id);const currentStock=beerData?beerData.stock:0;if(item.quantity<currentStock){cart[index].quantity+=1;updateCartCount();renderCart()}else{showNotification(`No hay más stock disponible de ${item.name} ${item.subname}`)}}
