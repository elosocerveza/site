...
if(product.sold&&parseInt(product.sold)>20){this.app.products.mostSold.push(product)}
this.addToCategory(product,category)});this.ensureMinimumProducts()}
addToCategory(product,category){const categoryMap={'beers':'beers','sauces':'sauces','preserves':'preserves','combos':'combos'};const targetCategory=categoryMap[category]||'beers';this.app.products[targetCategory].push(product)}
...
if(this.app.products.mostSold.length===0){this.app.products.mostSold=this.app.products.beers.slice(0,6).map(p=>({...p,sold:Math.floor(Math.random()*100)+50}))}}
