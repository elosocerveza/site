...
async function loadBeersData(){try{console.log('Cargando datos de cervezas...');const response=await fetch('https://script.google.com/macros/s/AKfycbw8ftCNEw6hJBslOS4hyTNniPefNlA_Zsu5b8EO_NcUJmnGXfnuWJNL5tPUAdMsmt4PCw/exec');const data=await response.json();console.log('Datos recibidos:',data);let beerArray=data;if(!Array.isArray(data)){const keys=Object.keys(data);for(const key of keys){if(Array.isArray(data[key])){beerArray=data[key];break}}}
if(!Array.isArray(beerArray)){throw new Error('Los datos recibidos no son un array')}
beersData=beerArray.map(item=>{let subcategory=item.subcategory||'otros';subcategory=subcategory.toLowerCase();return{id:item.id||0,name:item.name||'',subname:item.subname||'',description:item.description||'',price:parseInt(item.price)||0,category:'beers',subcategory:subcategory,image:item.image||'images/products/beers/default.jpg',stock:parseInt(item.stock)||0,sold:parseInt(item.sold)||0,rating:parseFloat(item.rating)||0,active:!0,abv:parseFloat(item.abv)||0,ibu:parseInt(item.ibu)||0,ingredients:item.ingredients||'',size:item.size||''}});console.log('Cervezas cargadas:',beersData.length);if(beersData.length>0){generateProductSchema(beersData)}
if(isBeersPage){renderBeers()}}catch(error){console.error('Error cargando datos de cervezas:',error);if(isBeersPage){const container=document.getElementById('beers-container');if(container){container.innerHTML=`
...
function getProductBadge(beer){if(beer.stock===0){return'<div class="product-badge">AGOTADO</div>'}else if(beer.stock<5){return'<div class="product-badge">ÚLTIMAS</div>'}else if(beer.sold>200){return'<div class="product-badge">POPULAR</div>'}else if(beer.rating>=4.8){return'<div class="product-badge">TOP</div>'}
return''}
...
document.addEventListener('DOMContentLoaded',function(){loadBeersData();initLazyLoading();const menuToggle=document.getElementById('menu-toggle');const menuClose=document.getElementById('menu-close');const mobileMenu=document.getElementById('mobile-menu');if(menuToggle)menuToggle.addEventListener('click',toggleMobileMenu);if(menuClose)menuClose.addEventListener('click',toggleMobileMenu);if(mobileMenu){mobileMenu.addEventListener('click',(e)=>{if(e.target===mobileMenu)toggleMobileMenu();})}
...
const whatsappMessage=buildWhatsAppOrderMessage();const phoneNumber='5491123495971';const encodedMessage=encodeURIComponent(whatsappMessage);const whatsappURL=`https://wa.me/${phoneNumber}?text=${encodedMessage}`;window.open(whatsappURL,'_blank');showNotification('Redirigiendo a WhatsApp para completar tu pedido')})}
