...
const{subtotal,totalSavings}=this.calculateCartTotals();const message=this.buildCompleteWhatsAppMessage(subtotal,totalSavings);if(this.app.managers.analytics){this.app.managers.analytics.trackPurchase('ELOSO_'+Date.now(),this.app.cart,subtotal)}
await this.submitCompleteOrderToAppsScript(subtotal);this.app.managers.ui.openWhatsApp(message);setTimeout(()=>{this.clearCart();this.resetCheckout();this.app.managers.ui.closeCartSidebar()},1000)}
