...
class ProductManager{constructor(app){this.app=app;this.CACHE_KEY='elOsoProductsCache';this.CACHE_DURATION=30*60*1000}
async loadProducts(){console.log('📥 Cargando productos desde Google Sheets...');const cachedData=this.getProductsFromCache();if(cachedData){console.log('✅ Productos cargados desde cache');this.processProductsData(cachedData);this.loadProductsFromNetwork();return}
await this.loadProductsFromNetwork()}
async loadProductsFromNetwork(){console.log('🌐 Cargando productos desde la red...');if(this.app.managers.analytics){this.app.managers.analytics.trackEvent('Products','load_start','google_sheets')}
for(const proxy of CONFIG.PROXIES){try{const sheetUrl=this.getSheetUrl();const proxyUrl=proxy+encodeURIComponent(sheetUrl);console.log(`🔧 Probando proxy: ${proxy}`);const response=await this.fetchWithTimeout(proxyUrl,8000);if(response.ok){const csvText=await response.text();console.log('✅ CSV obtenido correctamente desde la red');this.saveProductsToCache(csvText);this.processProductsData(csvText);return}}catch(error){console.warn(`❌ Proxy falló: ${error.message}`);continue}}
console.error('❌ Todos los proxies fallaron');if(!this.getProductsFromCache()){this.loadFallbackProducts()}}
...
parseCSV(csvText){const lines=csvText.split('\n').filter(line=>line.trim());if(lines.length===0)throw new Error('CSV vacío');const headers=lines[0].split(',').map(h=>h.trim().toLowerCase());const products=[];for(let i=1;i<lines.length;i++){if(!lines[i].trim())continue;try{const values=this.parseCSVLine(lines[i]);const product=this.createProductFromCSV(headers,values);if(product.id&&product.name&&product.category){products.push(product)}}catch(error){console.warn(`❌ Error procesando línea ${i}:`,error)}}
...
createRecommendationHTML(product){const displayPrice=product.discountPrice||product.price;return `
        ...
        `}
checkout(){if(this.app.cart.length===0){this.app.managers.ui.showNotification('❌ Tu carrito está vacío','error');return}
...
async submitOrderToForms(subtotal){const orderId='ELOSO_'+Date.now()+'_'+Math.random().toString(36).substr(2,9).toUpperCase();if(this.app.managers.forms){const orderData={orderId:orderId,items:this.app.cart.map(item=>({id:item.id,name:item.name,quantity:item.quantity,price:item.discountPrice||item.price,total:(item.discountPrice||item.price)*item.quantity})),total:subtotal,phone:'A confirmar por WhatsApp',address:'A coordinar por WhatsApp',notes:`Pedido web - ${this.app.cart.length} productos`};try{await this.app.managers.forms.submitOrderToGoogleForm(orderData);if(this.app.managers.analytics){this.app.managers.analytics.trackPurchase(orderId,this.app.cart,subtotal)}}catch(error){console.error('❌ Error enviando pedido a Forms:',error)}}}
