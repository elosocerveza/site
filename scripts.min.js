...
nextStep(step){if(step===3&&!this.validateStep2()){return}
document.querySelectorAll('.checkout-step').forEach(stepEl=>{stepEl.classList.remove('active')});const nextStepEl=document.getElementById(`step${step}`);if(nextStepEl){nextStepEl.classList.add('active')}
...
updateStepUI(step){switch(step){case 2:this.updateRecommendations();break;case 4:this.updateOrderSummary();break}}
validateStep2(){return!0}
validateAndNext(){if(this.currentStep===3){if(this.validateCustomerForm()){this.saveCustomerData();this.nextStep(4)}}}
validateCustomerForm(){const form=document.getElementById('customerForm');const requiredFields=form.querySelectorAll('[required]');let isValid=!0;requiredFields.forEach(field=>{if(!field.value.trim()){field.style.borderColor='var(--accent-red)';isValid=!1;this.app.managers.ui.showNotification(`❌ Por favor completa el campo: ${field.previousElementSibling?.textContent || 'requerido'}`,'error')}else{field.style.borderColor=''}});const phoneField=document.getElementById('customerPhone');if(phoneField&&phoneField.value.trim()){const phoneRegex=/^[0-9\s\-\+\(\)]{8,}$/;if(!phoneRegex.test(phoneField.value)){phoneField.style.borderColor='var(--accent-red)';this.app.managers.ui.showNotification('❌ Por favor ingresa un número de teléfono válido','error');isValid=!1}}
return isValid}
...
updateOrderItems(){const container=document.getElementById('orderSummaryItems');if(!container)return;container.innerHTML=this.app.cart.map(item=>{const displayPrice=item.discountPrice||item.price;const totalPrice=displayPrice*item.quantity;return `
            ...
            `}).join('')}
updateOrderTotals(){const{subtotal,totalSavings}=this.calculateCartTotals();const subtotalEl=document.getElementById('summarySubtotal');const shippingEl=document.getElementById('summaryShipping');const totalEl=document.getElementById('summaryTotalFinal');if(subtotalEl)subtotalEl.textContent=`$${subtotal.toLocaleString()}`;if(totalEl)totalEl.textContent=`$${subtotal.toLocaleString()}`;if(this.customerData.deliveryDate==='jueves'||this.customerData.deliveryDate==='viernes'){if(shippingEl)shippingEl.textContent='GRATIS'}else{if(shippingEl){shippingEl.textContent='A CONSULTAR';shippingEl.style.color='var(--text-light)'}}}
