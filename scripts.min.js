...
async init(){try{this.setupManagers();await this.loadProducts();this.setupEventListeners();this.startCountdowns();this.loadCart();this.updateCartUI();console.log('🛒 El Oso - Ecommerce inicializado correctamente')}catch(error){console.error('❌ Error inicializando la app:',error)}}
...
class CartManager{constructor(app){this.app=app;this.currentStep=1;this.customerData={};this.initCheckout()}
...
const summaryFixed=document.getElementById('cartSummaryFixed');if(summaryFixed){summaryFixed.className=`step${step}`}}
...
return isValid}
saveCustomerData(){const form=document.getElementById('customerForm');const formData=new FormData(form);this.customerData={name:formData.get('name')||'',phone:formData.get('phone')||'',email:formData.get('email')||'',address:formData.get('address')||'',deliveryDate:formData.get('deliveryDate')||'',deliveryTime:formData.get('deliveryTime')||'',notes:formData.get('notes')||''}}
...
updateCustomerSummary(){const fields={'summaryCustomerName':this.customerData.name,'summaryCustomerPhone':this.customerData.phone,'summaryCustomerAddress':this.customerData.address,'summaryDeliveryDate':this.getDeliveryDateDisplay(this.customerData.deliveryDate),'summaryDeliveryTime':this.getDeliveryTimeDisplay(this.customerData.deliveryTime),'summaryCustomerNotes':this.customerData.notes||'Sin notas adicionales'};Object.entries(fields).forEach(([id,value])=>{const element=document.getElementById(id);if(element){element.textContent=value}});const notesRow=document.getElementById('summaryNotesRow');if(notesRow&&!this.customerData.notes){notesRow.style.display='none'}}
getDeliveryDateDisplay(dateValue){const dates={'jueves':'Jueves (Envío Gratis)','viernes':'Viernes (Envío Gratis)','otro':'Otro día (Consultar costo)'};return dates[dateValue]||'A confirmar'}
...
const{subtotal,totalSavings}=this.calculateCartTotals();const confirmationData={orderId:'ELOSO_'+Date.now(),cart:[...this.app.cart],customerData:{...this.customerData},subtotal:subtotal,totalSavings:totalSavings,timestamp:new Date().toISOString()};sessionStorage.setItem('elOsoOrderConfirmation',JSON.stringify(confirmationData));if(this.app.managers.analytics){this.app.managers.analytics.trackPurchase(confirmationData.orderId,this.app.cart,subtotal)}
await this.submitCompleteOrderToAppsScript(subtotal);this.clearCart();this.resetCheckout();this.app.managers.ui.closeCartSidebar();setTimeout(()=>{window.location.href='confirmation.html'},500)}
...
updateCartUI(){this.updateCartItems();this.updateCartSummary();this.app.managers.ui.updateShippingProgressStep1()}
...
calculateShippingCost(){const{subtotal}=this.calculateCartTotals();const isFreeShippingDay=this.customerData.deliveryDate==='jueves'||this.customerData.deliveryDate==='viernes';const qualifiesForFreeShipping=isFreeShippingDay&&subtotal>=CONFIG.FREE_SHIPPING_THRESHOLD;if(qualifiesForFreeShipping){return 0}else{return null}}}
